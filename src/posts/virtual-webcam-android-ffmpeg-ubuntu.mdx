...
Like the name suggests it streams the video over http so you can receive it from any device on the same network. For the best performance you can use USB tethering and the latency will be basically zero.
...
And that's it!sudo modprobe v4l2loopback devices=1 max_buffers=2 exclusive_caps=1 card_label="Virtual Webcam".
